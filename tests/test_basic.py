...
from termai import config as termai_config
...
def test_debug_config_redaction(monkeypatch, capsys):
    ...
    monkeypatch.setattr("termai.cli.load_config", lambda: fake_config)
...
def test_prompt_parsing(monkeypatch):
    ...
    monkeypatch.setattr("termai.cli.load_config", lambda: {"provider": "gemini"})
    ...
    monkeypatch.setattr("termai.cli.send_request", mock_send_request)
    cli.cli_entry_point()
...
def test_reinstall_logic(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("termai.cli.CONFIG_FILE", test_config)
    
    monkeypatch.setattr(sys, "argv", ["ai", "--reinstall"])
    # Mock load_config to avoid interactive setup
    monkeypatch.setattr("termai.cli.load_config", lambda: {})
